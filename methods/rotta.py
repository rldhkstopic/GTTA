...
class RoTTA(TTAMethod):
    def __init__(self, model, optimizer, img_resize, device, crop_size, steps, episodic, num_classes=14, mem_size=64, lambda_t=1.0, lambda_u=1.0, nu=0.9, update_frequency=64):
        ...
        # setup the ema model
        self.model_ema = deepcopy(self.model)               # <- Out of memory when Resize is larger.
        ...
        self.transform = get_carla_transforms(self.img_resize, gaussian_std=0.005, soft=True)
    ...
    @staticmethod
    def update_ema_variables(ema_model, model, nu): # -> Teacher Model update 과정
